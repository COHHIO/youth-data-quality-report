...
During the period from `r format(period_start_date, "%m/%d/%Y")` to `r format(period_end_date, "%m/%d/%Y")` there were:
...
n_duplicated_enrollment_ids_string <- ifelse(
    ...
    "✅ There were no duplicated enrollment IDs.",
    paste0("❌ There were ", n_duplicated_enrollment_ids, " duplicated enrollment IDs.")
)
...
if (length(distinct_n_hoh_per_h) == 1 && distinct_n_hoh_per_h == 1) {
    cat("✅ All households had exactly one head of household.")
} else {
    if (0 %in% distinct_n_hoh_per_h) {
        cat("❌ Some households didn't have a head of household.")
    }
    if (any(distinct_n_hoh_per_h > 1)) {
        cat("❌ Some households had more than one head of household.")
    }
}
```
...
if (n_exits_without_an_enrollment_record == 0) {
    cat("✅ All exits were associated with an enrollment.")
} else {
    cat(glue::glue("❌ { n_exits_without_an_enrollment_record } weren't associated with an enrollment."))
}
```
...
```{r check-health-conditions-disabilities-record-match-enrollment}
#| results: asis
n_disabilities_records_at_entry_without_enrollment_record <- data_health_conditions_and_disabilites_entry |>
    dplyr::anti_join(
        y = entries_in_period,
        by = c("enrollment_id", "personal_id", "organization_id")
    ) |>
...
if (n_disabilities_records_at_entry_without_enrollment_record == 0) {
    cat("✅ All records collected at entry matched an enrollment record.")
} else {
    cat(paste0(
        "❌ ",
        n_disabilities_records_at_entry_without_enrollment_record,
        " records collected at entry didn't match an enrollment record."
    ))
}
```

```{r check-enrollment-record-match-health-conditions-disabilities}
#| results: asis
n_enrollments_without_health_conditions_record <- entries_in_period |>
    dplyr::anti_join(
        y = data_health_conditions_and_disabilites_entry,
        by = c("enrollment_id", "personal_id", "organization_id")
    ) |>
    dplyr::distinct(enrollment_id, personal_id, organization_id) |>
    nrow()

if (n_enrollments_without_health_conditions_record == 0) {
    cat("✅ All enrollments had a health conditions and disabilities record.")
} else {
    cat(paste0(
        "❌ ",
        n_enrollments_without_health_conditions_record,
        " enrollments were missing a health conditions and disabilities record."
    ))
}
```

```{r check-health-conditions-disabilities-record-match-exit}
#| results: asis
n_disabilities_records_at_exit_without_exit_record <- data_health_conditions_and_disabilites_exit |>
    dplyr::anti_join(
        y = exits_in_period,
        by = c("enrollment_id", "personal_id", "organization_id")
    ) |>
...
if (n_disabilities_records_at_exit_without_exit_record == 0) {
    cat(
        "✅ All records collected at exit matched an exit record."
    )
} else {
    cat(paste0(
        "❌ ",
        n_disabilities_records_at_exit_without_exit_record,
        " records collected at exit didn't match an exit record."
    ))
}

```

```{r check-exit-record-match-health-conditions-disabilities}
#| results: asis
n_exits_without_health_conditions_record <- exits_in_period |>
    dplyr::anti_join(
        y = data_health_conditions_and_disabilites_exit,
        by = c("enrollment_id", "personal_id", "organization_id")
    ) |>
    dplyr::distinct(enrollment_id, personal_id, organization_id) |>
    nrow()

if (n_exits_without_health_conditions_record == 0) {
    cat("✅ All exits had a health conditions and disabilities record.")
} else {
    cat(paste0(
        "❌ ",
        n_exits_without_health_conditions_record,
        " exits were missing a health conditions and disabilities record."
    ))
}
```
...
```{r check-education-record-match-enrollment}
#| results: asis
n_education_records_at_entry_without_enrollment_record <- data_education_entry |>
    dplyr::anti_join(
        y = entries_in_period,
        by = c("enrollment_id", "personal_id", "organization_id")
    ) |>
...
if (n_education_records_at_entry_without_enrollment_record == 0) {
    cat("✅ All records collected at entry matched an enrollment record.")
} else {
    cat(paste0(
        "❌ ",
        n_education_records_at_entry_without_enrollment_record,
        " records collected at entry didn't match an enrollment record."
    ))
}
```

```{r check-enrollment-record-match-education}
#| results: asis
n_enrollments_without_education_record <- entries_in_period |>
    dplyr::anti_join(
        y = data_education_entry,
        by = c("enrollment_id", "personal_id", "organization_id")
    ) |>
    dplyr::distinct(enrollment_id, personal_id, organization_id) |>
    nrow()

if (n_enrollments_without_education_record == 0) {
    cat("✅ All enrollments had an education record.")
} else {
    cat(paste0(
        "❌ ",
        n_enrollments_without_education_record,
        " enrollments were missing an education record."
    ))
}
```

```{r check-health-conditions-education-record-match-exit}
#| results: asis
n_education_records_at_exit_without_exit_record <- data_education_exit |>
    dplyr::anti_join(
        y = exits_in_period,
        by = c("enrollment_id", "personal_id", "organization_id")
    ) |>
...
if (n_education_records_at_exit_without_exit_record == 0) {
    cat("✅ All records collected at exit matched an exit record.")
} else {
    cat(paste0(
        "❌ ",
        n_education_records_at_exit_without_exit_record,
        " records collected at exit didn't match an exit record."
    ))
}
...
```{r check-exit-record-match-education}
#| results: asis
n_exits_without_education_record <- exits_in_period |>
    dplyr::anti_join(
        y = data_education_exit,
        by = c("enrollment_id", "personal_id", "organization_id")
    ) |>
    dplyr::distinct(enrollment_id, personal_id, organization_id) |>
    nrow()

if (n_exits_without_education_record == 0) {
    cat("✅ All exits had an education record.")
} else {
    cat(paste0(
        "❌ ",
        n_exits_without_education_record,
        " exits were missing an education record."
    ))
}
```
...
data_employment_entry |>
    create_data_quality_table(
        ...
        group_column = "is_hoh_and_or_adult",
        group_column_label = "Head of Household and/or Adult",
        table_title = "Employment Status"
    )
```

```{r check-employment-type-responses}
data_employment_entry |>
    create_data_quality_table(
        ...
        group_column = "is_hoh_and_or_adult",
        group_column_label = "Head of Household and/or Adult",
        table_title = "Employment Type"
    )
```

```{r check-not-employed-reason-responses}
data_employment_entry |>
    create_data_quality_table(
        ...
        group_column = "is_hoh_and_or_adult",
        group_column_label = "Head of Household and/or Adult",
        table_title = "Not Employed Reason"
    )
```

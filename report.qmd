...
There were `r nrow(entries_in_period)` enrollments between `r period_start_date` and `r period_end_date`.
...
The `r nrow(entries_in_period)` enrollments in this period came from `r dplyr::n_distinct(data_enrollment_client$household_id)` distinct households.
